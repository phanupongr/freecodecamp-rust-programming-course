...
enum ImageDataErrors {
    ...
    BufferTooSmall,
    UnableToReadeImageFromPath(std::io::Error),
...
impl FloatingImage {
    fn new(width: u32, height: u32, name: String) -> Self {
        let buffer_capacity = width * height * 4; // 3_655_744;
...
fn find_image_from_path(path: String) -> Result<(DynamicImage, ImageFormat), ImageDataErrors> {
    match Reader::open(&path) {
        ...
        Err(e) => Err(ImageDataErrors::UnableToReadeImageFromPath(e)),
    }
}

fn get_smallest_image(dim_1: (u32, u32), dim_2: (u32, u32)) -> (u32, u32) {
...
fn standardise_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, height) = get_smallest_image(image_1.dimensions(), image_2.dimensions());
